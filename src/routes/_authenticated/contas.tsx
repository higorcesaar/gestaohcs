...
interface Account {
  id: string; name: string; bank: string | null; type: string; balance: number; color: string | null; titular: string | null;
}
...
function Contas() {
  ...
  const [form, setForm] = useState({ name: "", bank: "", type: "corrente", balance: "", titular: "" });
  ...
  async function save() {
    ...
    const { error } = await supabase.from("accounts").insert({
      ...
      type: form.type, balance: Number(form.balance) || 0,
      titular: form.titular || null,
    });
    if (error) return toast.error(error.message);
    setOpen(false); setForm({ name: "", bank: "", type: "corrente", balance: "", titular: "" });
    toast.success("Conta criada"); load();
  }
            ...
            <TableBody>
              ...
              {items.map((a) => (
                <TableRow key={a.id}>
                  ...
                  <TableCell className="text-right font-medium">{formatBRL(Number(a.balance))}</TableCell>
                  <TableCell><Button size="sm" variant="ghost" onClick={() => del(a.id)}><Trash2 className="size-4" /></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <Input placeholder="Nome (ex: Conta Corrente Inter)" value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} />
            ...
            <Select value={form.titular} onValueChange={(v) => setForm((p) => ({ ...p, titular: v }))}>
              <SelectTrigger><SelectValue placeholder="Titular" /></SelectTrigger>
              <SelectContent>{TITULARES.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
            </Select>
            <Input placeholder="Saldo inicial" type="number" value={form.balance} onChange={(e) => setForm((p) => ({ ...p, balance: e.target.value }))} />
          </div>
